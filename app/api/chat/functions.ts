...
type FunctionNames =
    |"cush_getV3Pool"|"cush_liveBlock"|"cush_search"|"cush_getPoolFees"|
    "cush_poolLiquidity"|"cush_topTokens"|"cush_ordersForUser";

export const functions:
    {name: FunctionNames; description : string; parameters : object;}[] = [
      {
        name : "cush_getV3Pool",
        ...
        parameters : {
          ...
          properties : {
            searchText : {
              type : "string",
              description :
                  `Search text query given by users. For single match, accepted formats for search text are token names, symbols, token Address, pool Address. For pair match, must provide either a " ", "/", or "-" in between pair items`,
            },
          },
          required : [ "searchText" ],
        },
...
async function cush_getV3Pool(
  ...
  // Implement the logic to get V3 pool snapshot address based on the provided
  // API endpoint
...
async function cush_getTokenAmounts(target_pool: string, block: number) {
  // Implement the logic to get token amounts in a pool at a given block based
  // on the provided API endpoint
}

async function cush_getPositionByTokenId(tokenId: number) {
  // Implement the logic to get position by token ID based on the provided API
  // endpoint
}

async function cush_getToken(token: string, block: number) {
  // Implement the logic to get token information based on the provided API
  // endpoint
}

async function cush_getTokenLevel1(token: string, block: number) {
  // Implement the logic to get token level 1 information based on the provided
  // API endpoint
}

async function cush_getTokensFromPoolAddress(addr: string) {
  // Implement the logic to get tokens from pool address based on the provided
  // API endpoint
}

async function cush_getPoolsContainingToken(target_token: string) {
  // Implement the logic to get pools containing token based on the provided API
  // endpoint
}
...
async function cush_getAllNewPositionsCreated(
  ...
  // Implement the logic to get all new positions created and show percentage
  // change based on the provided API endpoint
}

async function cush_getAllNewTransactions(timestamp: number, lookback: number) {
  // Implement the logic to get all new transactions and show percentage change
  // based on the provided API endpoint
}
...
async function cush_getNewPositionsCreatedByPool(
  ...
  // Implement the logic to get new positions created by pool and show
  // percentage change based on the provided API endpoint
}
...
async function cush_getNewTransactionsByPool(
  ...
  // Implement the logic to get new transactions by pool and show percentage
  // change based on the provided API endpoint
}
